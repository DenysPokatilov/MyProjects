        ...
        public void Run(string[] args)
        {
            ...
            do
            {
                 ...
                 */
                    ...
                    catch (Exception ex) { Console.WriteLine(ex.Message); } // Зачем?
                    /*
                     * Ты ни в одном из своих методов исключение не бросаешь, чтобы его здесь обрабатывать.
                     * Также не вижу, чтобы где-то могло ожидаться исключение в ситуации, не зависящей от 
                     * логики твоего кода и действий пользователя (например, при попытки записи в файл, 
                     * который в этот же момент может использоваться какой-то другой программой и быть 
                     * залоченным на изменения). 
                     * 
                     * Таким образом, этот try catch здесь ни к чему. Просто так, "на всякий случай", 
                     * какой-то код писать не надо. В качестве аналогии, это всё равно, что ты бы
                     * установил батарею снаружи квартиры за окном. Как бы вроде никому плохо не 
                     * делает, но вот только зачем? ;)
                     * 
                     * Более того, такой код не просто бесполезен, а даже вреден, так как только создаёт
                     * визуальный шум и отвлекает. Например, мне пришлось потратить какое-то время, чтобы
                     * просмотреть все твои методы в поисках мест, где ты бы создавал исключение, но так 
                     * ничего и не найти.
                     */ 
                }
                DisplayTasksMenu();
